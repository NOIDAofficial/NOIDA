...
const TARGET_TABLE_KEYWORDS = {
  ...
  // ★v2.1.2 Bug O 修正: calendar キーワードを拡充
  calendar: /(予定|会議|ミーティング|打ち合わせ|打合せ|打ち合せ|面談|商談|アポ|アポイント|約束|スケジュール|飲み会|食事会|ランチ|ディナー|イベント|パーティー|セミナー|勉強会|出張|旅行|散歩|筋トレ|ジム|散髪|美容院|病院|歯医者)/,
  ideas: /(アイデア|企画|構想)/,
}
...
/**
 * ★v2.0.6: calendar title から時刻・日付表現を除去して正規化
 * 
 * datetime は別カラムに保存されるので、title に時刻が入ってると冗長。
 * LLM が「明日15時に定例」みたいに入れてきても、ここで「定例」に正規化。
 * 
 * 除去するもの:
 *   - 時刻: "14時", "14:00", "14時半", "14:30", "15時から"
 *   - 日付: "明日", "今日", "明後日", "来週", "今週", "月曜", ...
 *   - 月日: "4月21日", "4/21"
 *   - 付帯語: "に", "の", "から"(前後に時刻日付があった時のみ)
 */
function normalizeCalendarTitle(rawTitle: string): string {
  if (!rawTitle) return rawTitle
  
  let cleaned = rawTitle
    // 日付表現
    .replace(/明日|今日|明後日|昨日|来週|今週|先週/g, '')
  ...
  if (cleaned.length < 2) return rawTitle.trim()
...
async function saveStructuredMemory(
  ...
  if (skipCalendarInsert && preLLMAnalysis?.conflict_detection) {
    ...
    const rawTitle = cleanSaveValue(newCalendarTitle) || preLLMAnalysis.extracted_title || '予定'
    const cleanTitle = normalizeCalendarTitle(rawTitle)
    ...
    for (let i = 0; i < calendarItems.length; i++) {
      ...
      const rawCalendar = cleanSaveValue(
      ...
      // ★v2.0.6: title から時刻・日付表現を除去
      const cleanCalendar = normalizeCalendarTitle(rawCalendar)
      if (cleanCalendar !== rawCalendar) {
        console.log('🧹 [v2.0.6] calendar title 正規化:', { raw: rawCalendar, normalized: cleanCalendar })
      }
...
【★★★ save の絶対禁止 ★★★】
以下は絶対に出力しない:
- "null"、"undefined"、"なし"、"N/A"、"省略可"、"(省略可)"、"(なし)" などの文字列
- 括弧付きメタ注釈: "(ユーザーが言った時のみ)"、"(具体的に述べられた場合のみ)" など
- ISO 日時文字列: "2026-04-21T14:00:00Z" のような形式(title には絶対入れない)
- 曖昧タイトル単独: "会議" / "打ち合わせ" / "予定" / "MTG" / "アポ" を title にしない
  → これらはオーナーがタイトル補足したら初めて使える(例: "営業会議"、"定例打ち合わせ")

保存するものがない場合は、そのフィールドの値を **JSON の null**(文字列じゃなく) で返す:
  正: "calendar": null
  誤: "calendar": "null"  
  誤: "calendar": "(省略可)"

【save.calendar】
- ユーザーが予定を言ったら自然言語 title で埋める
- ★★title には時刻を入れない★★ 時刻は datetime 経由で保存される
  正: save.calendar: "営業会議"        (オーナー:「明日15時に営業会議」)
  誤: save.calendar: "明日15時の営業会議"
  誤: save.calendar: "15時の営業会議"
- 複数の予定を同時に言われた場合は配列で返す:
  save.calendar: ["田中さんとの会議", "犬の散歩"]
- 単一なら文字列: save.calendar: "営業会議"
- 曖昧題目単独(「会議」「打ち合わせ」など)の場合は null(システムが clarification する)

【save.task】
明確なタスク(やるべきこと)のみ。「〜しないと」「〜やらなきゃ」の時だけ。
...
■★reply の絶対禁止(原則14 再掲)★
- clarification 指示がある時に「入れた」「保存した」「追加した」と過去形で言わない
- executionNote/preLLMConflict がある場合は必ずその指示に従う
- DB が未更新なのに「した」と言うのは嘘応答(信頼崩壊)

■必ずJSON形式のみで返答(フィールド欠落禁止)
{
  "reply": "応答テキスト",
  "reason": "1行理由(省略可)",
  "hint": "一言進言(省略可)",
