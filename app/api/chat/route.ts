...
async function performPreLLMAnalysis(
  ...
  // ★Bug A: タイトルの明示性を階層化
  // 明示タイトル: 「犬の散歩」「田中さんとの会議」など具体的
  // 曖昧題目: 「打ち合わせ」「会議」「予定」など単体
  const trimmed = text.trim()
  const hasVagueTopic = VAGUE_TOPICS_CONTAINS.test(trimmed)
  
  // 明示性の判定: 曖昧題目以外の情報(人物・場所・具体名詞)があれば明示と見なす
  const hasExplicitTitle = 
    ...
    // 曖昧題目以外の固有名詞っぽいもの
    (trimmed.length >= 6 && !VAGUE_TOPICS.test(trimmed))
  ...
  // 重複検出
  const conflictDetection = isCalendarAdd
